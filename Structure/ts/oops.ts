...
class SmartCar {
    ...
    constructor(
        public brand: string,              // Anyone can read or change this
        private engineCode: string,        // Only SmartCar methods can access this
        protected safetyRating: string,    // SmartCar and subclasses can access this
        public readonly vin: string        // Read-only, set only once
    ) { }

    showSecretEngineCode(): void {
...
class BankAccount {
    private _balance: number = 0; // The underscore (_) is a standard convention for private backing fields
...
class Airplane implements Flyable {
    constructor(public altitudeLimit: number, public aircraftModel: string) { }
...
class UserTracker {
    static activeUsersCount: number = 0;
...
class GPS {
    findRoute(destination: string): void {
...
class LuxuryCar {
    ...
    driveTo(destination: string): void {
